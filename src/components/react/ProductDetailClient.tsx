...
export default function ProductDetailClient({ product, industries: INDUSTRIES, similarProducts: resolvedSimilarProducts }: ProductDetailClientProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 pt-28 pb-24 lg:pb-20 transition-colors duration-300">
      <div className="max-w-[1300px] mx-auto px-4">
        ...
        <div className="flex items-center flex-wrap gap-1 sm:gap-2 text-[9px] sm:text-xs font-bold text-slate-400 mb-6 sm:mb-8 uppercase tracking-widest">
          ...
          <ChevronRight className="w-3 h-3 shrink-0 hidden sm:block" />
          <span className="text-slate-900 hidden sm:inline truncate max-w-[120px] md:max-w-none">{product.name}</span>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-12">
          ...
          <div className="flex-1 space-y-12">
            ...
            <div className="space-y-6">
              <div className="border-b border-slate-200 pb-8">
                <div className="flex items-center gap-3 mb-4">
                  <a href={`/products?category=${categorySlug(product.category)}`} className="bg-slate-100 text-slate-600 text-[10px] font-bold uppercase tracking-widest px-2 py-1 rounded-sm hover:bg-orange-100 hover:text-orange-700 transition-colors cursor-pointer">
                    {product.category}
                  </a>
                  {product.compliance?.map((c: string) => (
                    <span key={c} className="flex items-center gap-1 text-[10px] font-semibold text-slate-500 border border-slate-200 px-2 py-1 rounded-sm">
                      <ShieldCheck className="w-3 h-3" /> {c}
                    </span>
                  ))}
                </div>

                <h1 className="text-3xl md:text-5xl font-sans font-bold text-slate-900 tracking-tight leading-tight mb-6">
                  {product.name}
                </h1>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {[
                    ...
                    ...(product.formula && product.formula !== 'Unspecified' ? [{ label: 'Chemical Formula', value: product.formula }] : []),
                    { label: 'Molecular Weight', value: product.mw && product.mw !== "N/A" ? (product.mw.includes('g/mol') ? product.mw : `${product.mw} g/mol`) : "N/A" }
                  ].map(pill => (
                    <div key={pill.label} className="bg-slate-50 border border-slate-200 p-4 rounded-sm flex flex-col justify-center">
                      <span className="text-[10px] font-semibold text-slate-500 uppercase tracking-widest mb-1">{pill.label}</span>
                      <div className="flex items-center justify-between">
                        <span className="text-[13px] font-mono font-medium text-slate-900 truncate">{pill.value}</span>
                        <button
              ...
              <div className="flex flex-wrap gap-2 pt-2">
                ...
                {product.industry.map((ind: string) => {
                  ...
                  return industryObj ? (
                    <a key={ind} href={`/industries/${industryObj.slug}`} className="bg-white text-slate-700 border border-slate-200 px-2.5 py-1 rounded-sm text-[11px] font-bold hover:bg-orange-50 hover:text-orange-700 hover:border-orange-300 transition-colors cursor-pointer">{ind}</a>
                  ) : (
                    <span key={ind} className="bg-white text-slate-700 border border-slate-200 px-2.5 py-1 rounded-sm text-[11px] font-bold">{ind}</span>
                  );
                })}
              </div>
            </div>
            ...
            {/* Scroll Navigation */}
            <div className="space-y-6">
              <div className="sticky top-[86px] z-40 bg-gray-50/90 backdrop-blur-md border-b border-slate-200 flex gap-8 overflow-x-auto no-scrollbar pt-2">
                {sections.map(section => (
                  ...
                  >
                    ...
                    {activeSection === section.id && (
                      <motion.div layoutId="nav-indicator" className="absolute bottom-0 left-0 right-0 h-[2px] bg-slate-900" />
                    )}
                  </button>
                ))}
              </div>
              ...
              <div className="space-y-12">
                {/* Description Section */}
                {product.description && (
                <div id="description" className="bg-white p-8 border border-slate-200 scroll-mt-36">
                  <h3 className="text-xl font-bold text-slate-900 mb-6">Description</h3>
                  <div className="prose prose-slate max-w-none">
                    ...
                    {product.otherNames && product.otherNames !== "N/A" && (
                      <div className="mt-8 p-5 bg-slate-50 border border-slate-200">
                        <h4 className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-2">Other Names (Synonyms)</h4>
                        <p className="text-slate-700 font-medium text-[13px]">{product.otherNames}</p>
                      </div>
                    )}

                    <h4 className="text-slate-900 font-bold text-lg mt-10 mb-5">Key Technical Features</h4>
                    <ul className="grid grid-cols-1 md:grid-cols-2 gap-y-3 gap-x-6 list-none p-0 text-[13px]">
                      {['High Purity Grade standard', 'Consistent Batch Quality', 'Full Regulatory & REACH Support', 'Global Logistics Network enabled'].map((f, i) => (
                        <li key={i} className="flex items-center gap-3 text-slate-700 border-b border-slate-100 pb-2">
                ...
                <div id="physical-properties" className="bg-white border border-slate-200 scroll-mt-36 overflow-hidden">
                  <div className="flex items-center gap-3 px-8 py-5 bg-gradient-to-r from-orange-50 to-orange-100/50 border-b border-orange-200">
                    ...
                    <h3 className="text-xl font-bold text-slate-900">Physical Properties</h3>
                  </div>
                    ...
                    {(() => {
                      ...
                      return parts.map((part: string, i: number) => {
                        ...
                        if (colonIdx > 0) {
                          ...
                          const icon = iconMap[label] || <Atom className="w-4 h-4 text-slate-400" />;
                          return (
                            <div key={i} className={`flex items-center gap-4 px-8 py-3.5 ${i % 2 === 0 ? 'bg-white' : 'bg-slate-50/70'} hover:bg-orange-50/40 transition-colors`}>
                              <span className="shrink-0">{icon}</span>
                              <span className="font-semibold text-slate-700 min-w-[160px] text-sm">{label}</span>
                              <span className="text-slate-900 text-sm font-medium">{value}</span>
                            </div>
                          );
                        }
                        return <div key={i} className="px-8 py-3.5 text-slate-600 text-sm">{part.trim()}</div>;
                      });
                    })()}
                  </div>
                </div>
                ...
                <div id="safety-handling" className="bg-white border border-slate-200 scroll-mt-36 overflow-hidden">
                  <div className="flex items-center gap-3 px-8 py-5 bg-gradient-to-r from-orange-50 to-orange-100/50 border-b border-orange-200">
                    <div className="w-9 h-9 rounded-lg bg-orange-600 flex items-center justify-center">
                      <AlertTriangle className="w-[18px] h-[18px] text-white" />
                    </div>
                    <h3 className="text-xl font-bold text-slate-900">Safety & Handling</h3>
                  </div>
                  {(() => {
                          ...
                          <div className="px-6 sm:px-8 py-5 flex flex-col sm:flex-row sm:items-center gap-4">
                            ...
                            {ghsCodes.length > 0 && (
                              <div className="flex flex-wrap gap-2">
                                {ghsCodes.map((code, i) => (
                                  <span key={i} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-orange-50 border border-orange-200 text-xs font-semibold text-orange-700">
                                    <ShieldCheck className="w-3.5 h-3.5" />
                                    {code}
                                    <span className="text-orange-500 font-normal">· {ghsNames[code] || 'Hazard'}</span>
                                  </span>
                                ))}
                              </div>
                            )}
                        ...
                        {hazardStatements.length > 0 && (
                          <div className="px-6 sm:px-8 py-5">
                            <div className="flex items-center gap-2 mb-3">
                              ...
                              <h4 className="text-sm font-bold uppercase tracking-wider text-red-600">Hazard Statements</h4>
                            </div>
                            <div className="grid gap-1.5">
                              {hazardStatements.map((h, i) => (
                                <div key={i} className={`flex items-start gap-3 px-4 py-2.5 rounded-lg ${i % 2 === 0 ? 'bg-red-50/50' : 'bg-white'}`}>
                                  <span className="shrink-0 mt-0.5 inline-flex items-center justify-center min-w-[52px] px-2 py-0.5 rounded bg-red-100 text-red-700 text-xs font-mono font-bold">
                                    {h.code}
                                  </span>
                                  <span className="text-sm text-slate-700 leading-relaxed">{h.desc}</span>
                                </div>
                              ))}
                            </div>
                          </div>
                        ...
                        {precStatements.length > 0 && (
                          <div className="px-6 sm:px-8 py-5">
                            <div className="flex items-center gap-2 mb-3">
                              ...
                              <h4 className="text-sm font-bold uppercase tracking-wider text-blue-600">Precautionary Statements</h4>
                            </div>
                            <div className="grid gap-1.5">
                              {precStatements.map((p, i) => (
                                <div key={i} className={`flex items-start gap-3 px-4 py-2.5 rounded-lg ${i % 2 === 0 ? 'bg-blue-50/40' : 'bg-white'}`}>
                                  <span className="shrink-0 mt-0.5 inline-flex items-center justify-center min-w-[52px] px-2 py-0.5 rounded bg-blue-100 text-blue-700 text-xs font-mono font-bold">
                                    {p.code}
                                  </span>
                                  <span className="text-sm text-slate-700 leading-relaxed">{p.desc}</span>
                                </div>
                              ))}
                            </div>
                          </div>
                ...
                <div id="trade-regulatory" className="bg-white border border-slate-200 scroll-mt-36 overflow-hidden">
                  <div className="flex items-center gap-3 px-8 py-5 bg-gradient-to-r from-orange-50 to-orange-100/50 border-b border-orange-200">
                    <div className="w-9 h-9 rounded-lg bg-orange-600 flex items-center justify-center">
                      <Scale className="w-[18px] h-[18px] text-white" />
                    </div>
                    <h3 className="text-xl font-bold text-slate-900">Trade & Regulatory</h3>
                  </div>
                  <div className="divide-y divide-slate-100">
                    {(() => {
                          ...
                          return (
                            <div key={i} className={`flex items-center gap-4 px-8 py-3.5 ${i % 2 === 0 ? 'bg-white' : 'bg-slate-50/70'} hover:bg-orange-50/40 transition-colors`}>
                              ...
                              <span className="text-sm font-medium">
                                {isHSCode ? (
                                  <span className="inline-flex items-center px-2.5 py-0.5 rounded bg-orange-100 text-orange-700 font-mono">{value}</span>
                                ) : isStorage ? (
                                  <span className="inline-flex items-center px-2.5 py-0.5 rounded bg-amber-50 text-amber-700 border border-amber-200">{value}</span>
                                ) : isWGK ? (
                                  <span className="inline-flex items-center px-2.5 py-0.5 rounded bg-blue-50 text-blue-700 border border-blue-200">{value}</span>
                                ) : (
                ...
                <div id="similar-products" className="bg-white p-6 sm:p-8 border border-slate-200 scroll-mt-36">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
                    {resolvedSimilarProducts.map((similarProduct) => (
                        <a
                          ...
                          className="group rounded-lg border border-slate-200 px-2.5 py-2 hover:border-orange-400 hover:bg-orange-50/40 transition-all"
                        >
                          <p className="text-xs font-semibold text-slate-800 group-hover:text-orange-600 transition-colors leading-snug line-clamp-1">{similarProduct.name}</p>
            ...
            <div className="sticky top-24 space-y-6">
              ...
              <div className="bg-white p-6 border border-slate-200 space-y-5">
                ...
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 bg-slate-100 flex items-center justify-center text-slate-600 font-bold rounded-sm">
                    SC
                  </div>
                  <div>
                    <p className="text-[13px] font-bold text-slate-900">SoleChem</p>
                    <p className="text-[10px] text-slate-500 uppercase font-semibold tracking-widest mt-0.5">Sector Specialist</p>
                  </div>
                </div>
                <div className="flex gap-2">
                  <a href="tel:+390230556150" className="flex-1 bg-slate-50 hover:bg-slate-100 rounded-sm p-2 flex items-center justify-center transition-colors border border-slate-200">
                    <Phone className="w-4 h-4 text-slate-600" />
                  </a>
                  <a href="mailto:info@solechem.eu" className="flex-1 bg-slate-50 hover:bg-slate-100 rounded-sm p-2 flex items-center justify-center transition-colors border border-slate-200">
      ...
      {!isQuoteModalOpen && (
      <div className="fixed bottom-0 left-0 right-0 z-50 lg:hidden bg-white/95 backdrop-blur-md border-t border-slate-200 px-4 py-3 safe-bottom">
        <button
          onClick={() => setIsQuoteModalOpen(true)}
